...
pub fn param_u64(name: &str) -> u64 {
    ...
    {
        let result =
            unsafe { host_shapes::env_shape_param_u64(name.as_ptr(), name.len() as u32) };
        if result < 0 {
...
pub fn param_i64(name: &str) -> i64 {
    ...
    {
        let result =
            unsafe { host_shapes::env_shape_param_i64(name.as_ptr(), name.len() as u32) };
        if result == i64::MIN {
...
pub fn param_bool(name: &str) -> bool {
    ...
    {
        let result =
            unsafe { host_shapes::env_shape_param_bool(name.as_ptr(), name.len() as u32) };
        if result < 0 {
...
pub fn param_string(name: &str) -> String {
    #[cfg(target_arch = "wasm32")]
    {
        ...
        let len = unsafe {
            host_shapes::env_shape_param_string(
                name.as_ptr(),
                name.len() as u32,
                buf.as_mut_ptr(),
            )
...
pub fn param_bytes(name: &str) -> Vec<u8> {
    #[cfg(target_arch = "wasm32")]
    {
        ...
        let len = unsafe {
            host_shapes::env_shape_param_bytes(
                name.as_ptr(),
                name.len() as u32,
                buf.as_mut_ptr(),
            )
...
pub fn param_u128(name: &str) -> u128 {
    #[cfg(target_arch = "wasm32")]
    {
        let mut buf = [0u8; 16];
        let result = unsafe {
            host_shapes::env_shape_param_u128(
                name.as_ptr(),
                name.len() as u32,
                buf.as_mut_ptr(),
            )
...
pub fn time_in(shape_name: &str) -> u64 {
    ...
    {
        let result = unsafe {
            host_shapes::env_shape_time_in(shape_name.as_ptr(), shape_name.len() as u32)
        };
        if result < 0 {
            return 0;
        }
